...
proxy_auth = {
    'username': get_env_or_default('PROXY_UNAME', ''),
    'password': get_env_or_default('PROXY_PASS', ''),
    'type': int(get_env_or_default('PROXY_TYPE', 0)),
    'hostname': get_env_or_default('PROXY_HNAME', ''),
    'port': int(get_env_or_default('PROXY_PORT', 8080))
}

proxies = {
    'peer': proxy_auth.copy(),
    'web_seed': proxy_auth.copy(),
    'dht': proxy_auth,
    'tracker': proxy_auth
}
